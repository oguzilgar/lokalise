...
Cypress.Commands.add(
  ...
  (userEmail, userPassword) => {
    ...
    cy.get(`input[placeholder='user@company.com']`)
      .clear()
      .type(`${userEmail}`);
...
Cypress.Commands.add(`logOut`, () => {
  ...
  cy.contains(`Create new team`).should(`be.exist`).should(`be.visible`);

  cy.contains(`Profile settings`).should(`be.exist`).should(`be.visible`);

  cy.contains(`Logout`).should(`be.exist`).should(`be.visible`);
...
Cypress.Commands.add(
  `is_projectName_displayed_on_editorPage`,
  (userFullName) => {
    cy.get(`a`)
      .contains(`Projects`)
      .next()
...
Cypress.Commands.add(`is_projectsPage_displayed`, () => {
  cy.get(`div[class='nav-item active']`).should(`be.visible`);
